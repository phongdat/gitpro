        ...
        foreach ( scandir($uploads_dir) as $yfolder ) {
            ...
            foreach ( scandir("$uploads_dir/$yfolder") as $mfolder ) {
                ...
                foreach ( $images as $image ) {
                    ...
                    if ( ! preg_match( '/([0-9]{1,4})x([0-9]{1,4})(@2x)?.(jpg|jpeg|png|gif)/', $filename ) ) continue;
...
if ( ! function_exists( 'yit_remove_protocol_url' ) ) {
    /**
     * Force the URL to https://, if we are in SSL
     *
     * @since 1.0.0
     */
    function yit_remove_protocol_url( $url ) {
